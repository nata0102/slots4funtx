                ...
                <div class="form-group">
                  ...
                  <select id="machine" class="form-control selectpicker @error('machine_id') is-invalid @enderror input100" name="machine_id" required="" data-live-search="true">
                      ...
                      @foreach($machines as $machine)
                        <option class="hidden {{ $machine->permission ? 's-'.$machine->permission->lkp_type_permit_id : '' }}" value="{{$machine->id}}"  {{ old('machine_id') == $machine->id ? 'selected' : '' }}>{{$machine->id}} - {{$machine->owner->value}} - {{$machine->serial}}</option>
                      @endforeach
                  </select>
                  @error('machine_id')
                      <span class="invalid-feedback" role="alert">
